...
export default function AnuncioForm({
  ...
  // Debug: log render and user state
  useEffect(() => {
    console.log("[AnuncioForm] Rendered. User:", user?.id, "Device:", navigator.userAgent.includes("Mobile") ? "Mobile" : "Desktop");
  }, [user?.id]);
  ...
  const { data: anunciantesData, error: anunciantesError } = useQuery({
    queryKey: ["anunciantes-do-usuario", user?.id],
    queryFn: async () => {
      try {
        console.log("[AnuncioForm] Fetching anunciantes for user:", user?.id);
        ...
        if (user?.tipoUsuario === "adm") {
          ...
          console.log("[AnuncioForm] Anunciantes fetched (admin):", data?.length);
          return data;
        }
        ...
        if (!response.ok) throw new Error("Erro ao buscar anunciantes");
        const data = await response.json();
        console.log("[AnuncioForm] Anunciantes fetched (user):", data?.length);
        return data;
      } catch (error) {
        console.error("[AnuncioForm] Error fetching anunciantes:", error);
        throw error;
      }
    },
    enabled: !!user,
  ...
  useEffect(() => {
    const errors: string[] = [];
    if (anunciantesError) {
      errors.push("Erro ao carregar anunciantes");
      console.error("[AnuncioForm] Anunciantes error:", anunciantesError);
    }
    if (anuncioError) {
      errors.push("Erro ao carregar anúncio");
      console.error("[AnuncioForm] Anuncio error:", anuncioError);
    }
    setLoadingErrors(errors);
  }, [anunciantesError, anuncioError]);
  ...
  // Show loading errors
  if (loadingErrors.length > 0 && !anuncioId) {
    console.warn("[AnuncioForm] Loading errors detected:", loadingErrors);
    return (
    ...
    <div className="min-h-screen bg-vitrii-gray-light py-12">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          {/* Show warning if no anunciantes loaded */}
          {anunciantes.length === 0 && !anunciantesError && (
            <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                ⏳ Carregando seus anunciantes...
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4 pt-6 border-t">
              <Link
                to="/sell"
                className="px-4 py-3 border-2 border-vitrii-blue text-vitrii-blue rounded-lg font-semibold hover:bg-blue-50 transition-colors text-center"
              >
                Cancelar
              </Link>
